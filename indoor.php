...
if (isset($_POST['add_to_cart'])) {
    ...
    if ($prod && $prod['stock_status'] == 'in_stock') {
        ...
        header("Location: indoor.php?success=1");
        exit();
    } else {
...
// Fetch Fresh Indoor Plants
$stmt = $pdo->prepare("SELECT * FROM products WHERE category_id = (SELECT id FROM categories WHERE name = 'Fresh Indoor') AND active_status = 'active'");
$stmt->execute();
...
<head>
    ...
    <title>Fresh Indoor Plants - Fresh-Factor</title>
...
<body class="bg-[#F9FBE7]">
    ...
    <div class="container mx-auto px-6 py-12">
        <h1 class="text-4xl font-bold text-[#1B5E20] mb-4 text-center"><?php echo htmlspecialchars(t('fresh_indoor_plants')); ?></h1>
        <p class="text-gray-600 text-center mb-12"><?php echo htmlspecialchars(t('fresh_indoor_plants_subtitle')); ?></p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
            <?php foreach ($plants as $plant): ?>
                <div class="bg-white rounded-3xl shadow-xl overflow-hidden border border-green-100 flex flex-col hover:shadow-2xl transition duration-300">
                    <a href="product.php?id=<?php echo $plant['id']; ?>" class="block focus:outline-none focus:ring-2 focus:ring-[#43A047] focus:ring-offset-2">
                        <div class="h-56 sm:h-64 overflow-hidden relative">
                            <img src="<?php echo htmlspecialchars(asset_url($plant['image_url'] ?: 'assets/placeholder_plant.png')); ?>" class="w-full h-full object-cover" alt="<?php echo htmlspecialchars($plant['name']); ?>">
                            <div class="absolute top-4 right-4 bg-[#1B5E20] text-white px-4 py-2 rounded-full font-bold text-sm shadow-lg">
                                <?php echo $plant['price']; ?> SAR
                            </div>
                        </div>
                        <div class="p-6 sm:p-10 flex-grow flex flex-col">
                            <h2 class="text-xl sm:text-2xl font-bold text-gray-800 mb-3 sm:mb-4 line-clamp-1"><?php echo htmlspecialchars($plant['name']); ?></h2>
                            <p class="text-gray-600 text-sm sm:text-base mb-6 leading-relaxed line-clamp-3"><?php echo htmlspecialchars($plant['description']); ?></p>
                        </div>
                    </a>
                    ...
                    <div class="px-6 sm:px-10 pb-8 flex-grow flex flex-col">
                        ...
                        <div class="bg-gray-50 rounded-2xl p-6 mb-8 border border-dashed border-green-200">
                            <h4 class="font-bold text-[#1B5E20] mb-3 text-sm flex items-center">
                                <i class="fas fa-tint mr-2 text-blue-400"></i> CARE INSTRUCTIONS
                            </h4>
                            <p class="text-xs text-gray-500 italic">"Low maintenance, thrives in medium light. Water once a week."</p>
                            <?php if ($plant['stock_status'] == 'in_stock'): ?>
                                <form method="POST">
                                    <input type="hidden" name="product_id" value="<?php echo $plant['id']; ?>">
            ...
            <?php if (empty($plants)): ?>
                <div class="col-span-full text-center py-20 bg-white rounded-3xl border border-dashed">
                    <p class="text-gray-400 italic">Fresh Indoor Plants coming soon!</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
